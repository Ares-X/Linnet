...
已经完成过上述首次安装的用户，后续只使用对应 `core-v<version>` 页面的 Core 安装包；更新不会删除或重新添加输入源，也不需要再次注销。安装程序只更新磁盘文件，不停止正在运行的 Host，因此不会断开 TextEdit、Teams、浏览器或其他应用的输入连接，也不要求退出这些应用。安装后打开 **Settings → 数据 → 版本**，可分别查看“已安装”和“正在运行”的版本与 revision；现有 Host 会继续安全运行，macOS 下次正常启动 Linnet 时自动使用已安装的新 Core。
