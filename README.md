...
Linnet 不会自动点击系统授权或选择输入源；首次安装后的允许与每次选择始终由用户和 macOS 完成。已有 App 的 Complete 修复与日常 Core 更新都不会注册、启用或选择输入源，因此不会因更新重复弹出授权。若输入菜单中的 Linnet 已被 macOS 移除或停用，请在 **系统设置 → 键盘 → 文本输入 → 编辑** 中重新添加或启用；安装器不会绕过这项系统授权。社区安装包没有 Apple Developer ID 签名或公证，因此“无法验证开发者”是预期提示；手动信任只应在文件名和同一 Release 说明中的 SHA-256 都校验成功后进行。不要关闭 Gatekeeper、不要清除隔离属性，也不要执行来源不明的安装命令。若系统报告校验和不一致、文件损坏或恶意软件，请停止安装。
