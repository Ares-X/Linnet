...
本文对应 **0.1.10**；正式可下载版本以 Latest Release 为准。
本次重点改进候选文字与释义布局、主题预览、候选翻页及预测编号，并修正语言数据更新误报。
完整内容见[0.1.10 变更记录](CHANGELOG.md#0110--2026-08-30)。
...
本地语言数据比频道版本更新时，会明确显示“已安装的语言数据领先于此更新频道”，不会提示降级或下载旧包。语言包以各自的 Data release 序号比较新旧；整组更新必须兼容且不让任何已有包倒退，同一序号对应不同内容会报告校验失败。
...
输出必须与 Core 更新页说明中的 SHA-256 完全一致。随后在 Finder 中按住 Control 点击 Core 安装包并选择 **打开**。Core 只更新 App，保持已安装语言包、个人数据和输入源状态，不重新注册输入源。如果 Linnet Settings 正打开，安装器会先关闭这个 Linnet 自身窗口；不需要退出 TextEdit、Teams、浏览器等正在使用的应用。
...
健康安装不要用 `Linnet.pkg` 覆盖 Core。只有 Core 明确报告 Linnet App 身份受支持但 TIS 注册缺失时，Complete 才负责修复注册；重复、冲突或无法验证的系统残留必须先使用官方卸载工具，再重新安装 Complete。不要手工复制 App 或绕过检查。
...
当前公开版本见 [Latest Release](https://github.com/Ares-X/Linnet/releases/latest)；本分支版本为 **0.1.10**（2026-08-30）。完整的用户可见变化见[版本记录](CHANGELOG.md)。
