...
中文候选支持横排或竖排、每页 3/5/7/9 项以及滚动或临时展开浏览。数字键可直接选择候选，鼠标、方向键、Page Up / Page Down、`[` / `]`、`-` / `=` 与候选窗按钮负责翻页。没有组合或候选菜单时，`/ , . ; ' [ ] - =` 都直接交给当前应用；有候选时 `[` / `]`、`-` / `=` 才作为翻页键，拼音方案自身的音节分隔键仍按该方案工作。
...
2. 在 macOS 安装器中依次选择 **继续 → 安装**并等待“安装成功”。只有首次创建 `Linnet.app` 时，Complete 才注册输入源并向 macOS 提交一次启用请求；只要 App 已存在，Complete 与 Core 更新都不会重新注册、启用或选择输入源。Linnet 只安装到当前用户的 `~/Library/Input Methods/Linnet.app`；不要把 App 手工拖动或复制到其他目录。
...
Linnet 不会自动点击系统授权或选择输入源；首次安装后的允许与每次选择始终由用户和 macOS 完成。已有 App 的 Complete 修复与日常 Core 更新都不会注册、启用或选择输入源，因此不会因更新重复弹出授权。Complete 在安装器内部也只以普通载荷传递更新内容，不会再暴露另一个可被系统识别的 `Linnet.app`；系统始终只看到 `~/Library/Input Methods/Linnet.app` 这一份身份。若输入菜单中的 Linnet 已被 macOS 移除或停用，请在 **系统设置 → 键盘 → 文本输入 → 编辑** 中重新添加或启用；安装器不会绕过这项系统授权。社区安装包没有 Apple Developer ID 签名或公证，因此“无法验证开发者”是预期提示；手动信任只应在文件名和同一 Release 说明中的 SHA-256 都校验成功后进行。不要关闭 Gatekeeper、不要清除隔离属性，也不要执行来源不明的安装命令。若系统报告校验和不一致、文件损坏或恶意软件，请停止安装。
...
Keychain 密码；安装脚本核对包内固定身份和完整目标内容，不读取维护者 Keychain，
也不会因为用户没有信任开发证书而把完好的 App 误报为损坏。若看到 Keychain
密码提示，应取消并向项目报告。正式
...
输出必须与 Core 更新页说明中的 SHA-256 完全一致。随后在 Finder 中按住 Control 点击 Core 安装包并选择 **打开**。Core 基于已公布版本的 App 应用差分，校验完整目标后原子替换，保持已安装语言包、个人数据和输入源状态，不重新注册输入源，也不关闭 Settings、TextEdit、Teams、浏览器等窗口。Settings 正在修改数据时，安装会暂缓，请完成该操作后重试。安装基线不匹配时，Core 不会自动改用全量覆盖；按明确提示使用 Complete 安装包修复。
...
**两行版本号相同，为什么仍提示有更新？** 展开 **高级版本详情**（**Advanced version details**），比较两行“源码修订”。本地验收修复可能沿用同一产品版本和 build，但源码修订不同；这时旧代码仍在运行，需要按上述步骤应用更新。
...
正常升级只使用 Core；只有基线不匹配、App 缺失或 App 字节损坏时，才使用完整 `Linnet.pkg`。Complete 保留健康的已安装词包与个人数据；若 App 已存在，它也不会触碰输入源状态。注册缺失或停用由 **系统设置 → 键盘 → 文本输入 → 编辑** 处理，而不是让安装器重复申请授权。重复、冲突或无法验证的系统残留必须先使用官方卸载工具，再重新安装 Complete。不要手工复制 App 或绕过检查。
...
当前公开版本见 [Latest Release](https://github.com/Ares-X/Linnet/releases/latest)；本分支版本为 **0.1.11**（2026-09-02）。完整的用户可见变化见[版本记录](CHANGELOG.md)。
