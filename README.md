...
已经完成过上述首次安装的用户，后续只使用对应 `core-v<version>` 页面的 Core 安装包；更新不会删除或重新添加输入源，也不需要再次注销。安装程序只更新磁盘文件，不停止正在运行的 Host，因此不会断开 TextEdit、Teams、浏览器或其他应用的输入连接，也不要求退出这些应用。安装后打开 **Settings → 数据 → 版本**，可分别查看“已安装”和“正在运行”的版本与 revision。可以继续使用当前 Host；若页面提供“应用已安装的更新”，可按提示先从 macOS 菜单切换到其他输入法并关闭本次登录期间使用过 Linnet 的其他应用，再由用户主动应用。Linnet 不会替用户关闭应用或程序化切换输入源；无法证明安全时会保留当前 Host，新 Core 在下次登录或重启后生效。
...
| 数据 | 组件版本、iCloud Drive 学习词同步、完整恢复归档、数据导入导出、学习数据清理、备份恢复与隐私处理后的诊断 |
...
Settings 每次打开时会从同一 GitHub 仓库读取一个经过校验的小型 Catalog，并在“数据 → 版本”中安静显示“已是最新”、Core 更新或语言数据更新；同一区域还会比较磁盘上已安装的 Core 与当前 Host 的版本、build 和 revision，避免把“安装成功”误报成“新代码已经运行”。输入进程始终离线；检查不会下载大型词库，也不会弹出系统通知。只有用户明确点击语言数据更新后，Settings 才会下载并校验语言包，再通过现有事务原子激活。
...
输出必须与 Core 更新页说明中的 SHA-256 完全一致。随后在 Finder 中按住 Control 点击 Core 安装包并选择 **打开**。Core 只更新 App，保持已安装语言包、个人数据、输入源状态和现有应用的输入连接，不重新注册输入源，也不要求再次注销或退出正在使用的应用。安装结束后在 **Settings → 数据 → 版本** 检查“已安装”和“正在运行”的身份；两者暂时不同是预期状态。可以继续使用当前 Host，也可以按页面提示主动应用；安全条件不满足时不会退出 Host，新 Core 在下次登录或重启后生效。已有安装不要使用 `Linnet.pkg`；Complete 会拒绝覆盖并提示改用 Core。若 Core 报告身份、版本或安装状态不一致，请停止，不要手工复制 App 或改用 Complete 绕过检查。
