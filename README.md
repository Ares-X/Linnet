...
已经完成过上述首次安装的用户，后续只使用对应 `core-v<version>` 页面的 Core 安装包；更新不会删除或重新添加输入源，也不需要再次注销。为避免让仍连接旧 Host 的应用出现键盘失效，安装程序只更新磁盘文件，不会停止或强制替换正在运行的 Host。安装后打开 **Settings → 数据 → 版本**：这里会分别显示“已安装”和“正在运行”的版本与 revision；切换到其他输入源并关闭仍连接旧 Host 的应用后，可点按 **Apply Installed Update**，Settings 只在 Host 确认空闲时让它自行退出，从固定安装路径启动新 Host，并核对新 revision。
...
输出必须与 Core 更新页说明中的 SHA-256 完全一致。随后在 Finder 中按住 Control 点击 Core 安装包并选择 **打开**。Core 只更新 App，保持已安装语言包、个人数据、输入源状态和现有应用的输入连接，不重新注册输入源，也不要求再次注销。安装结束后在 **Settings → 数据 → 版本** 检查“已安装”和“正在运行”的身份；需要立即应用时，先切换到其他输入源并按页面提示关闭仍连接旧 Host 的应用，再点按 **Apply Installed Update**。如果仍有输入客户端或数据事务，Host 会拒绝退出，不会强杀或假报成功；系统下次自然启动 Host 仍会读取磁盘上的新 Core。已有安装不要使用 `Linnet.pkg`；Complete 会拒绝覆盖并提示改用 Core。若 Core 报告身份、版本或安装状态不一致，请停止，不要手工复制 App 或改用 Complete 绕过检查。
...
当前版本是 **0.1.8**（2026-08-27）。完整的用户可见变化见[版本记录](CHANGELOG.md)。
