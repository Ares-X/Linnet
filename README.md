...
正常升级优先使用 Core；只有基线不匹配、App 缺失或输入源注册缺失等明确修复提示时，才使用完整 `Linnet.pkg`。Complete 保留健康的已安装词包与个人数据，已注册的输入源不会重复注册。重复、冲突或无法验证的系统残留必须先使用官方卸载工具，再重新安装 Complete。不要手工复制 App 或绕过检查。
