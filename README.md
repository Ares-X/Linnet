...
5. 点击菜单栏输入菜单或按系统的输入源切换快捷键，选择带小鸟图标的 **Linnet**。这一步只选择 Linnet 输入源，不是在系统列表中分别添加中文和英文。
6. 确认菜单栏显示 `中`/`双`，输入拼音可以提交中文候选；独立轻按 Shift 应显示 `En` 并输入英文，Caps Lock 应显示 `A`；输入菜单中的 **Settings** 应能打开设置窗口。

已经完成过上述首次安装的用户，后续只使用对应 `core-v<version>` 页面的 Core 安装包；更新不会删除或重新添加输入源，也不需要再次注销。安装程序只更新磁盘文件，不停止正在运行的 Host，因此不会断开 TextEdit、Teams、浏览器或其他应用的输入连接，也不要求退出这些应用。如果 Linnet Settings 正在打开，macOS Installer 会先关闭这一个 Linnet 自身窗口；安装完成后重新打开 Settings 即可看到新状态。随后在 **Settings → Data & Updates → Core update**，可分别查看“已安装”和“正在运行”的版本与 build；完整源码修订只在“Advanced version details / 高级版本详情”中显示。可以继续使用当前 Host；若页面提供“Apply Installed Update / 应用已安装的更新”，只需先从 macOS 菜单切换到其他输入法，用户应用保持打开。Linnet 不会替用户关闭应用或程序化切换输入源；无法证明 Host 处于安全状态时会保留当前 Host，新 Core 在下次正常登录或重启后生效。
...
| 输入 | 中文区管理全拼/双拼、学习、简繁、Emoji、标点、辅助码和反查；Smart English 区管理自动大写、IPA、释义、预测、纠错、学习、Space 尾随空格和 Tab |
| 词典 | 自定义词、禁用英文词与 Text Expander |
| 数据与更新 | Core 与词包版本、iCloud Drive 增量学习同步、手动完整备份/恢复、事务恢复记录、导入导出、学习数据清理与隐私处理后的诊断 |
...
输出必须与 Core 更新页说明中的 SHA-256 完全一致。随后在 Finder 中按住 Control 点击 Core 安装包并选择 **打开**。Core 只更新 App，保持已安装语言包、个人数据、输入源状态和现有应用的输入连接，不重新注册输入源，也不要求再次注销或退出正在使用的应用。安装结束后在 **Settings → Data & Updates → Core update** 检查“已安装”和“正在运行”的身份；两者暂时不同是预期状态。可以继续使用当前 Host，也可以在切换到其他输入法后按页面提示主动应用；安全条件不满足时不会退出 Host，新 Core 在下次正常登录或重启后生效。健康安装不要用 `Linnet.pkg` 覆盖 Core。只有 Core 明确报告 Linnet App 身份受支持但 TIS 注册缺失时，Complete 才负责修复注册；重复、冲突或无法验证的系统残留必须先使用官方卸载工具，再重新安装 Complete。不要手工复制 App 或绕过检查。
...
当前版本是 **0.1.9**（2026-08-28）。完整的用户可见变化见[版本记录](CHANGELOG.md)。
