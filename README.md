...
已经完成过上述首次安装的用户，后续只使用对应 `core-v<version>` 页面的 Core 安装包；更新完成后原来的 Linnet 输入源和当前输入连接继续保留，无需删除、重新添加、重启正在输入的应用或再次注销。为避免打断应用，安装程序不会启动、隐藏或替换正在运行的 Linnet Host，候选框和模式提示保持可见；磁盘上的新 Core 会在 macOS 下一次自然启动 Host 时接管。
...
Settings 每次打开时会从同一 GitHub 仓库读取一个经过校验的小型 Catalog，并在“数据 → 版本”中安静显示“已是最新”、Core 更新或语言数据更新。输入进程始终离线；检查不会下载大型词库，也不会弹出系统通知。只有用户明确点击语言数据更新后，Settings 才会下载并校验语言包，再通过现有事务原子激活。
...
输出必须与 Core 更新页说明中的 SHA-256 完全一致。随后在 Finder 中按住 Control 点击 Core 安装包并选择 **打开**。Core 只更新 App，保持已安装语言包、个人数据、输入源状态和现有应用的输入连接，不重新注册输入源，也不要求再次注销。当前 Host 会继续服务，磁盘上的新 Core 在 macOS 下一次自然启动 Host 时生效。已有安装不要使用 `Linnet.pkg`；Complete 会拒绝覆盖并提示改用 Core。若 Core 报告身份、版本或安装状态不一致，请停止，不要手工复制 App 或改用 Complete 绕过检查。
