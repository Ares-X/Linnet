...
2. 在 macOS 安装器中依次选择 **继续 → 安装**并等待“安装成功”。Complete 会在输入源缺失时注册，并通过 macOS 标准 API 启用、选中 Linnet 一次，使它进入当前用户的输入菜单；已有正确记录不会重复注册。Linnet 只安装到当前用户的 `~/Library/Input Methods/Linnet.app`；不要把 App 手工拖动或复制到其他目录。
...
5. Complete 通常已经选中带小鸟图标的 **Linnet**；确认它出现在菜单栏输入菜单中。之后可通过菜单或系统输入源快捷键切换。这只是一个 Linnet 输入源，不需要分别添加中文和英文。
...
Linnet 不会自动点击系统授权；Complete 只在用户明确运行完整安装/修复时选中 Linnet 一次，日常 Core 更新不会注册、启用或选择输入源。社区安装包没有 Apple Developer ID 签名或公证，因此“无法验证开发者”是预期提示；手动信任只应在文件名和同一 Release 说明中的 SHA-256 都校验成功后进行。不要关闭 Gatekeeper、不要清除隔离属性，也不要执行来源不明的安装命令。若系统报告校验和不一致、文件损坏或恶意软件，请停止安装。
...
正常升级优先使用 Core；只有基线不匹配、App 缺失、输入源禁用或注册缺失等明确修复提示时，才使用完整 `Linnet.pkg`。Complete 保留健康的已安装词包与个人数据，已注册的输入源不会重复注册，并通过一次标准启用/选择修复状态栏菜单记录。重复、冲突或无法验证的系统残留必须先使用官方卸载工具，再重新安装 Complete。不要手工复制 App 或绕过检查。
