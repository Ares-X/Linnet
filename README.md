...
2. 在 macOS 安装器中依次选择 **继续 → 安装**并等待“安装成功”。Complete 只在输入源缺失时注册；若 macOS 还没有保存当前用户的启用授权，它会提交一次标准授权请求。看到“允许 Linnet 启用 Linnet？”时请选择 **允许**。已有正确记录不会重复注册或请求授权。Linnet 只安装到当前用户的 `~/Library/Input Methods/Linnet.app`；不要把 App 手工拖动或复制到其他目录。
...
Linnet 不会自动点击系统授权或选择输入源；Complete 只在用户明确运行完整安装/修复且 macOS 尚未保存启用授权时提交一次标准请求，最终允许与选择都由用户完成。日常 Core 更新不会注册、启用或选择输入源。社区安装包没有 Apple Developer ID 签名或公证，因此“无法验证开发者”是预期提示；手动信任只应在文件名和同一 Release 说明中的 SHA-256 都校验成功后进行。不要关闭 Gatekeeper、不要清除隔离属性，也不要执行来源不明的安装命令。若系统报告校验和不一致、文件损坏或恶意软件，请停止安装。
...
正常升级优先使用 Core；只有基线不匹配、App 缺失、注册缺失或 macOS 明确要求重新授权等修复提示时，才使用完整 `Linnet.pkg`。Complete 保留健康的已安装词包与个人数据，已注册的输入源不会重复注册；缺少启用授权时只提交一次系统请求，用户允许后再从输入菜单选择 Linnet。重复、冲突或无法验证的系统残留必须先使用官方卸载工具，再重新安装 Complete。不要手工复制 App 或绕过检查。
