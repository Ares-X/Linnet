...
当前正式版为 **[0.1.23（105）](https://github.com/Ares-X/Linnet/releases/tag/v0.1.23)**，包含模糊音、邻键纠错及相关修复，并简化了少量配置项的展示。各版本的功能与变化见[版本记录](CHANGELOG.md)。

**[0.1.25 Preview（107）](https://github.com/Ares-X/Linnet/releases/tag/core-v0.1.25)** 包含连续中英混输增强，以及导出文件名和界面语言修复。已有用户可在 **Settings → 数据与更新 → 预览版** 中下载并应用 Core 更新。
...
- **中文里连续混输（Preview）**：直接输入“跨region的migration”这类句子，英文整词与两侧中文一起组成候选，无需中途切换模式。
...
中文默认提供邻键误按和前后鼻音的纠错建议。原编码能组成完整读音时，首选保留原读音，读音纠错优先于邻键纠错。例如自然码 `hghk` 的首选仍对应 `heng hao`，后面依次建议“很好”“更好”；全拼按完整拼音处理。

邻键纠错处理每个音节内的一次误按，多个音节可以分别纠错；正常双拼编码和简拼继续可用。

在 Settings → 输入的中文方案下方展开“模糊音”，可按声母、韵母选择 z/zh、n/l、in/ing 等 12 组读音混用，默认不勾选。折叠时显示已选组合；勾选后读音混用不再仅作为弱纠错候选，仍保留原读音，点击“应用更改”后生效。
...
**连续中英混输（0.1.24 起的 Preview）**：保持中文模式，按当前全拼或双拼方案输入中文，英文单词直接敲原文，中途不必上屏或切换模式。英文整词可以参与同一句中的多次中英切换，例如：
...
_0.1.25 Preview 在 macOS 虚拟机 Safari 文本框中的真实按键、原生候选窗与上屏过程；使用自然码双拼与每页 3 项候选，通过自动按键录制，原速播放。全拼与另外六种双拼按各自中文编码输入，英文拼写相同。_

`size`、`mode`、`save` 等也可能对应中文读音的词，会提供可选择的英文候选，同时保留中文候选。开启中文学习时，选过的混输词句及英文分界可以在全拼和双拼之间复用。候选仍按上下文与学习排序，有歧义时可自行选词。

按住 Shift 输入 `CPU`、`DNS`、`HTTPS` 等全大写缩写时，缩写保持原样，左右两侧的拼音继续组成中文句子。
...
核心与词包分离后，日常程序升级可复用大体积语言数据。语言包更新优先使用差分，未变化的词包直接复用；没有可用差分或差分失败时，自动下载对应的完整词包。
...
请从项目的 **[Latest Release](https://github.com/Ares-X/Linnet/releases/latest)** 只下载一个文件：`Linnet.pkg`。如需核对下载文件，可进入下载目录计算哈希：

```bash
shasum -a 256 Linnet.pkg
```

如需核对下载文件，输出应与同一 Release 说明中的 SHA-256 一致。Linnet 安装到当前用户目录，不需要管理员权限，也不会安装守护进程、启动项或特权辅助程序。

### 首次启用

1. 在 Finder 中按住 Control 点击（或右键点击）下载的 `Linnet.pkg`，选择 **打开**，再次确认 **打开**。若系统没有显示该选项，请打开 **系统设置 → 隐私与安全性**，在 Linnet 的拦截提示旁选择 **仍要打开**，然后返回安装器。
...
Linnet 不会自动点击系统授权、选择或停用输入源。若 Linnet 被 macOS 移除或停用，请在 **系统设置 → 键盘 → 文本输入 → 编辑** 中重新添加或启用。社区安装包没有 Apple Developer ID 签名或公证，因此“无法验证开发者”是预期提示；请确认文件来自本项目 Release 后再手动信任。不要关闭 Gatekeeper、清除隔离属性或执行来源不明的安装命令；校验和不一致或系统报告文件损坏时应立即停止。
...
在“数据与更新”中选择 **正式版**（默认）或 **预览版**，查看程序和词库的已安装与可用版本。语言数据优先使用差分更新，未变化的词包直接复用；没有可用差分或差分失败时，自动下载有变化的完整词包，新数据准备好之前继续使用当前数据。

若提示同版本词包冲突，可点击 **修复语言数据更新**，直接下载所选频道中有变化或冲突的完整词包。修复保留学习词、个人设置和未变化的词包，不降级词包，也不需要重装输入法。
...
在线 Core 只提供更高版本升级；需要修复同版本 App 时，请使用完整 `Linnet.pkg`。没有待应用更新时，按钮置灰是正常的。有未保存的设置或正在进行的数据操作时，请先完成页面提示的操作。应用被阻止时，根据卡片说明处理后重试；Linnet 不会替你切换输入源或强制关闭用户应用。若旧版 Host 不支持当前会话内应用更新，会明确提示等下次正常登录或重启后生效，不应反复重装或删除输入源。支持会话内更新的 Host 无需注销。

固定 CMS 版本之间的正常升级只使用 Core；0.1.7 或更早的旧 ad-hoc 版本，以及 App 缺失、损坏或发布身份不符时，使用完整 `Linnet.pkg`。Complete 保留健康的已安装词包与个人数据；若 App 已存在，它也不会触碰输入源状态。注册缺失或停用由 **系统设置 → 键盘 → 文本输入 → 编辑** 处理，而不是让安装器重复申请授权。重装不要求先卸载或预先清理系统注册状态。
...
iCloud 学习词同步为可选功能，通过 `iCloud Drive/Linnet` 同步中英文学习记录。自动检查最多每小时一次，也可点击“立即同步”，同步期间可以继续输入，无需先使用中英文模式或保持输入文档开启。

设置显示本机上次成功合并与导出学习数据的时间，以及失败或待重试的状态。本机完成不代表其他 Mac 已经收到数据；跨设备传输仍由 iCloud Drive 完成。

自动同步只处理学习词，自定义词、停用词、Text Expander 和设置不随之同步。迁移或恢复时，可在“数据与更新”中手动上传、审阅恢复备份，或导入导出个人数据；导入需确认，并会先创建本地备份。云端备份没有可用历史时，上传会自动新增完整备份，已有备份不会被删除。
