...
中文候选支持横排或竖排、每页 3/5/7/9 项。选择“可展开”后，首次候选保持紧凑；使用 `[` / `]` 或 `-` / `=` 真正翻到其他候选页时，会自动展开最多三行。展开后可用左右键逐项移动、上下键在相邻行的同一列移动，只有越过当前可见范围时网格才整体推进。也可以选择始终按页滚动。
...
候选窗口提供宣纸、月华、青岩、陶印、雾青、原生玻璃和墨朱七套主题，每套都包含浅色与深色版本。中文与英文可以分别选择紧凑状态下的横排或竖排；展开后统一使用 macOS 式多行网格。英文释义在展开状态固定显示于网格下方，只随当前高亮候选更新。字体、字号、候选数量和展开方式也可以独立调整。
...
当前公开版本见 [Latest Release](https://github.com/Ares-X/Linnet/releases/latest)；本分支版本为 **0.1.15**（2026-09-04）。完整的用户可见变化见[版本记录](CHANGELOG.md)。
