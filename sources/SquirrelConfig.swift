...
private extension SquirrelConfig {
  ...
  func color(from colorStr: String, inSpace colorSpace: SquirrelTheme.RimeColorSpace) -> NSColor? {
    if let matched = try? /0x([A-Fa-f0-9]{2})([A-Fa-f0-9]{2})([A-Fa-f0-9]{2})([A-Fa-f0-9]{2})/.wholeMatch(in: colorStr) {
      ...
      guard let alpha = Int(alpha, radix: 16), let red = Int(red, radix: 16),
        let green = Int(green, radix: 16), let blue = Int(blue, radix: 16)
      else { return nil }
      ...
      guard let red = Int(red, radix: 16), let green = Int(green, radix: 16),
        let blue = Int(blue, radix: 16)
      else { return nil }
      return color(alpha: 255, red: red, green: green, blue: blue, colorSpace: colorSpace)
    } else {
      return nil
    }
  }
