...
ruby -e '
  ...
  windows = File.read(ARGV.fetch(5))
  manifest = File.read(ARGV.fetch(6))
  local_cache = "./.github/actions/restore-locked-build-cache"
  local_windows = "./.github/workflows/windows-build.yml"
  ...
  windows_uses = windows.scan(/^\s*uses:\s*(\S+)/).flatten
  all_uses = workflow_uses + commit_uses + pull_request_uses + cache_uses + windows_uses
  abort unless all_uses.all? { |value|
    value == local_cache || value == local_windows || value.match?(/@[0-9a-f]{40}\z/)
  }
  ...
  jobs = workflow.scan(/^  ([a-z][a-z0-9-]*):\n(.*?)(?=^  [a-z][a-z0-9-]*:\n|\z)/m)
  writers = jobs.select { |_, body| body.match?(/^\s*contents:\s*write\s*$/) }
  abort unless writers.map(&:first) == %w[stage-update-channels publish-stable]
...
' "${workflow}" "${commit_workflow}" "${pull_request_workflow}" "${cache_action}" \
    "${publisher}" "${windows_workflow}" "${asset_manifest}" ||
