...
| verified | P0 | At `b5cf850`, Core/manual activation stopped the existing IMKServer while TextEdit PID 20213 retained its old client endpoint. TextEdit then produced no Host activation or key event; fresh TextEdit PID 73247 immediately created a working connection to the replacement Host. Unicode Hex Input was the only enabled keyboard-layout fallback, so the disconnected client appeared to have a dead keyboard. | Core package Host lifecycle | The custom post-install Host replacement owner is retired. Core updates replace only the installed App bytes, leave the current Host and every InputMethodKit client connection untouched, perform no TIS mutation, and require neither app exit nor logout. Settings passively compares the process-start running identity with the installed identity; the installed Core becomes authoritative on the next normal Host start. The runtime-footprint guard rejects any return of the activation command, client-count readiness inference, Host termination, or Host launch path. Exact-package installed continuity remains a release acceptance row. |
