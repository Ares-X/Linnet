...
| J13 | Catalog-bound Chinese, English, LTS and Extended update, source selection, cancellation, rollback and offline failure | C, P, V, I, R | Current source/component gates require the Catalog to come directly from the canonical GitHub HTTPS endpoint; GitHub direct/default, the built-in GH-Proxy public mirror (third-party) and an advanced compatible HTTPS mirror may route pack bytes, with no automatic fallback. Catalog bytes/sequence plus container byte count/SHA bind transport identity; Registry owns manifest and file verification. Core 0.1.1 fixes the online Catalog floor at sequence 4 and rejects a lower Catalog before any pack request. No current package binds these bytes; the service remains typed `unpublished`, and real GitHub/GH-Proxy Linnet large-pack transfers, visible and installed failure journeys remain pending. |
| J14 | input process stays offline and Settings networking is user initiated | C, P, I | Source guards retain Settings as the only bounded network/route owner. No fresh current-tree App exists, so the six-Mach-O offline scan is `NOT_EXERCISED`; the previous result is not current-candidate evidence. The language-pack service remains `unpublished` and its UI disabled, while v1 Core/App has no background or in-app updater. A signed package projection, published-channel J13/J14 network journeys and installed observation are also `NOT_EXERCISED`. |
...
| closed | P1 | Settings inferred online-update availability from local Active data and an embedded public key, so an unpublished channel looked actionable. | typed Linnet data-channel service state | one `LinnetDataChannel.Service` owns unpublished/published status and the current source is explicitly unpublished with bilingual copy. A new package projection and visible behavior remain part of the open P/V/UAT rows. |
...
Until those fields are complete, README and release tooling must describe the
product as an unpublished release candidate, not a completed Beta.
