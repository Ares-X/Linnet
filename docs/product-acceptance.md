...
| J15 | Core upgrade, pack-only update and incompatible downgrade | P, I | Package/registry fixtures require Complete-only registration plus one logout. Core closes only Settings and never stops the live InputMethodKit Host or calls register/enable/select, so existing client connections remain authoritative while new bytes wait for a natural Host launch. Every exact artifact still requires the installed `两轮同 leaf Core` sequence: previous accepted fixed-CMS build to candidate, then byte-identical candidate reinstall, proving stable Host PID and input in both an already-connected and newly opened application; activation of the new build after a natural Host restart is a separate required row. |
