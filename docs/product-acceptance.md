...
| closed | P1 | The Host lifecycle CLI and postinstall treated routine Core updates as the user's Text Input session, enabling repeated stop/register/selection designs to disturb menu state and client connections. | Complete input-source repair and Core package lifecycle | Complete owns the only direct `TISRegisterInputSource` / `TISEnableInputSource` / `TISSelectInputSource` path and runs it only after the user explicitly chooses full install/repair. Core and uninstall have no product-process termination or TIS mutation owner. |
...
| closed | P0 | At the `e6f5da1` audit base, Core treated a missing App as an in-place repair while Complete rejected an absent App whenever safe owned Active/state residue remained; neither path could authoritatively repair a supported missing-App or missing-registration installation. | Complete registration and repair boundary plus package-owned read-only TIS classification | Core accepts only a supported App identity with one exact enabled/selectable source; a missing, disabled or conflicting source fails before mutation and directs the user to Complete. Complete accepts a clean absent App or supported signed App only when Active/state links are safe, then owns the one standard register-if-missing, enable and select transition. The count-only `registered` interpretation and `missing-app-install` Core transition are retired. Exact installed repair rows remain pending. |
