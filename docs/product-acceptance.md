...
| open | P0 | The Host lifecycle CLI and postinstall treated an immediate HIToolbox result as durable user authorization and selection, while local build copies duplicated the production bundle identity in LaunchServices. | Complete input-source authorization request and Core package lifecycle | Complete alone may register Linnet and submit one `TISEnableInputSource` request. macOS owns approval and menu selection; `TISSelectInputSource` has no product owner. Core and uninstall never mutate TIS, and the canonical local-build cleanup retires every non-installed same-identity LaunchServices record. Close only after real menu visibility, selection, multi-app typing, and a same-identity Core update produce no new prompt. |
