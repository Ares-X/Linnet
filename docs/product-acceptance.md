...
Selecting the technical term must likewise remain learnable. Verify partial
...
Transactions must share the live volume. Preserve v3 backup restore and the
explicit portable import/export codec as shipped external contracts, not as a
fallback for normal v4 snapshots. Pre-release v2 backups are unsupported and
must remain non-authoritative. Only selected imported/cleared languages are
...
a required local isolated-desktop PASS; without it no candidate receipt is issued.
The candidate Action does not repeat any source, Rime or Settings UI row.
...
merely raising the minimum Core to `0.1.10` is not sufficient. This wire
compatibility does not authorize runtime-identity inference: an identity-free
health reply remains unavailable and cannot expose Core activation.
...
Valid real-window evidence and controlled installed-product evidence
...
Core updates accept only an installed fixed-CMS App; public 0.1.8 is the minimum
direct Core-upgrade source. Public 0.1.7 and older ad-hoc Apps must use Complete,
...
| macOS product runners per full CI | quality + App + Settings UI + Swift + Rime (5) | local receipt owner (1); remote product-test runners (0) |
...
| Data & Updates | installed/running Core identity, always-present activation card, Catalog-bound pack update/cancel, GitHub direct/default, built-in GH-Proxy public mirror (third-party), advanced custom compatible HTTPS mirror, iCloud incremental learning sync, manual full backup/restore, transaction recovery history, edition repair, legacy import, learning reset, portable data and diagnostics | current source/component gates cover the selector, bilingual copy, no automatic fallback and one Catalog identity owner. Missing running identity is fail-closed unavailable and cannot expose Core activation; the expired version-specific bridge is retired. Visible failure recovery, standard keyboard/focus behavior, real mirror transfer and installed lifecycle evidence are pending |
...
| J17 | Settings keyboard navigation, focus order, labels and reduced-motion behavior | V, I | pending |
...
| J20 | artifact names, sizes, checksums, notices, SBOM, update URLs and README agree | P, R | The candidate Action accepts one clean exact-main revision and its complete local source receipt, signs once on macOS, and freezes the manifest's verifier-approved Draft Release files without repeating local product tests. Installation consumes that exact set before any public-channel mutation. Only the SSH control tag binding version, full revision and manifest set digest may let the Ubuntu publisher route Core/data/Catalog and the unchanged `Linnet.pkg` to public Release / Latest. Candidate Draft bytes and final Release-page evidence remain distinct R rows. |
