...
完整拼写的中文词条不再阻止 `ScriptTranslation` 生成同音组合。`Poet` 使用原来的
词图、词频和 LTS 模型；`ScriptTranslation` 只在组合的搭配分更好，且组成词的
联合词频分比整词高出 `log(100)` 时让组合领先。这个保守的词典先验是排序校准，
不是语义正确率：用于保住“时机成熟”“异径三通”“实践调研”等合法整词，
同时纠正“异径连接”挡住“已经＋连接”的情况。完整已学词继续优先；术语不删除。
比较使用原始整词首选；整词胜出时，
整组完整词的原有顺序均保留，避免后续低频同音词被组合逐个挤走。
缩写或未完成音节仍走原来的整词补全路径，
不提前展开大量可能的后缀。

词频搜索只比较词频，最终输出少量候选时才计算搭配分，避免对将被丢弃的路径反复查询模型。
词典广度优先查询将三层索引的路径分数直接存于固定数组，队列消费及结果转交使用移动语义；
长词仍通过原有索引尾部查询，不改变搜索深度、候选顺序或评分。该内部布局变化要求重建全部原生插件。
一次 `ScriptTranslator::Query` 中的中文、英文和原始拼写搜索，通过同一 Poet 按上下文、
词语和句尾标记复用模型分数。路径权重和学习权重仍分别相加；结果只存在于同步查询栈上，
不被延迟候选持有，也不跨按键、模型或配置变化保留。
普通组句也复用 Poet 已有的词频搜索分支，在 `max_sentences` 剩余位置补充
词频分不低于模型首句、且不切开原句词语的同音组合，不挤掉原有模型首句。
允许“已经＋失效”合为“已经失效”，不允许“选择＋四则”变为“选择死＋则”。
组句词图保留系统词频与学习权重的区别：已学词的排序仍用学习权重，
同音补选使用匹配系统词条的原始词频，避免经常选择“连接”后，“链接”组合消失。
这样“已经链接”及“这个链接已经失效了”能保留为完整备选；同音的“链接／连接”
仍需语境或用户选择区分。无需新增词库、模型、Settings 投影或过滤器排序 owner。

`tests/verify_rime_runtime.sh --chinese-phrase-ranking-probe` 运行独立语义集，
检查全拼和自然码首选、目标前三位、术语召回，以及八方案共享学习、完整与逐段选词、
退格、原文 Return、Escape 和继续输入。查询集先执行且不学习，交互学习随后执行。
修改这条路径同时验证 profile golden、grammar、Chinese learning、mixed input、
spelling 和每键延迟；真实键盘流程仍在专用 VM 验收。
